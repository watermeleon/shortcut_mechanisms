...
def _path_patch_single(
    ...
    if apply_metric_to_cache:
        _, cache = model.run_with_cache(orig_input, return_type=None, names_filter=names_filter_for_cache_metric, prepend_bos=False)
        model.reset_hooks()
        return patching_metric(cache)
    else:
        if isinstance(patching_metric, str):
            loss = model(orig_input, return_type="loss", loss_per_token=(patching_metric=="loss_per_token"), prepend_bos=False)
            model.reset_hooks()
            return loss
        else:
            logits = model(orig_input, prepend_bos=False)
            model.reset_hooks()
...
def path_patch(
    ...
    direct_includes_mlps: bool = True,
    verbose: bool = False,
    per_example_results: bool = False,
) -> Float[Tensor, "..."]:
    '''
    ...
    if orig_cache is None:
        _, orig_cache = model.run_with_cache(orig_input, return_type=None, prepend_bos=False)
    if new_cache == "zero":
        new_cache = ActivationCache({k: t.zeros_like(v) for k, v in orig_cache.items()}, model=model)
    elif new_cache is None:
        _, new_cache = model.run_with_cache(new_input, return_type=None, names_filter=relevant_names_filter, prepend_bos=False)
    ...
    path_patch_single = partial(
        ...
        new_cache=new_cache,
        # seq_pos=seq_pos,  #  seq_pos is implemented below
    ...
    # If we're fixing receiver(s), and iterating over senders:
        ...
        if per_example_results:
            return_dict = dict()
            for node_name, results in results_dict.items():
                assert not isinstance(results[0], float), "Can't return per-example results if results are floats - make sure metric returns metric per example."
                split_batch_list = [[t[i] for t in results] for i in range(results[0].shape[0])]
                batch_tensors = [t.tensor(batch_results).reshape(list(sender_nodes.shape_values[node_name].values())) for batch_results in split_batch_list]
                return_dict[node_name] = t.stack(batch_tensors, dim=0)
            return return_dict
        else:
            return {
...
def _act_patch_single(
    ...
    if apply_metric_to_cache:
        _, cache = model.run_with_cache(orig_input, return_type=None, prepend_bos=False)
        model.reset_hooks()
        return patching_metric(cache)
    else:
        if isinstance(patching_metric, str):
...
def act_patch(
    ...
    if new_cache == "zero":
        ...
        _, new_cache = model.run_with_cache(new_input, return_type=None, prepend_bos=False)
